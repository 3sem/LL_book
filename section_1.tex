...
\\
{\bf$\blacktriangle$ Определение 13.1.} 
...
{\bf$\blacktriangle$ Определение 13.5.}
...
{\bf$\blacktriangle$ Определение 13.6.}
...
{\bf$\blacktriangle$ Определение 13.7.}
...
\begin{flushleft}
	...
	\begin{tabular}  {p{2.20cm}  p{8.30cm}}
		{\bf\normalsize Лемма 13.3 (о построении управляющей таблицы)}
		& \emph{Для LL(k)-грамматики $G = \langle N, \Sigma, P, S \rangle$, для любого её правила вида $A \vdash X_1\dots{X_l} \in P$, если $x \in FIRST_k(FIRST_k(X_1)\cdot{}\dots{\cdot{}{FIRST_k(X_l)}}\cdot{FOLLOW_k(A)})$, то ячейка управляющей таблицы $T_k(A, x)$ содержит единственное правило $A \vdash X_1\dots{X_l}$.}.
	\end{tabular}
	
\end{flushleft}	


Теорема 13.3 утверждает, что для LL(k)-грамматики управляющая таблица может быть построена без возникновения конфликтов. Если же её условие приводит к противоречиям, то грамматика не является LL(k).
...
\begin{flushleft}
	...
	\begin{tabular}  {p{2.20cm}  p{8.30cm}}
		{\bf\normalsize Лемма 13.4 (об эквивалентном определении)}
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}
		...
		\State Для всех правил вида $A \vdash \alpha X \beta: FOLLOW(X) \gets FOLLOW(X) \cup (FIRST(\beta) \setminus \{\varepsilon\} )$.
		\State Для всех правил вида $A \vdash \alpha X \text{ и } A \vdash \alpha X \beta \text{, где } \varepsilon \in FIRST(\beta): FOLLOW(X) \gets FOLLOW(X) \cup FOLLOW(A)$
		\EndWhile
	\end{algorithmic}
\end{algorithm}
Алгоритм для вычисления множества $FOLLOW$ представлен в \ref{algo_follow}.

\textbf{$\blacktriangle$ Задача 13.1} Рассмотрим грамматику $G$ со следующими правилами:
\begin{itemize}
	\item $S  \vdash a S'$
...
\vspace{10pt}        
\textbf{$\blacktriangle$ Задача 13.2} Построить не-$LL(1)$ грамматику. 
\vspace{5pt}
\hbox{%
	\color[gray]{0.85}{ \vrule width 4.0mm } \hspace{.5em} \color[rgb]{0,0,0} \parbox{0.9\textwidth}%
	{{\bf Решение}
		
		Грамматика $S \vdash aS | a$, согласно теореме 13.4, не является LL(1)-грамматикой, так как $FIRST(aS)=FIRST(a)=\{a\}$ и $FIRST(aS)\cap{FIRST(a)}=\{a\}$, но LL(2)-грамматикой, так как $FIRST_2(aS)=\{aa\}$, $ FIRST_2(a)=\{a\}$, и $FIRST_2(aS)\cap{FIRST_2(a)}=\varnothing$ --- является.
		\medskip
...
\textbf{$\blacktriangle$ Задача 13.3} Построить таблицу для грамматики $S \vdash aSbS \mid \varepsilon$ 
\vspace{5pt}
\hbox{%
	\color[gray]{0.85}{ \vrule width 4.0mm } \hspace{.5em} \color[rgb]{0,0,0} \parbox{0.9\textwidth}%
	{{\bf Решение}
...
\begin{flushleft}
	...
	\begin{tabular}  {p{2.20cm}  p{8.30cm}}
		{\bf\normalsize Лемма 13.5 (об эквивалентном определении)}
		& \emph{Грамматика $G = \langle \Sigma, N, S, P \rangle$ и $\omega \in \Sigma^{*}$ является LL(1) тогда и только тогда, когда выполнены 2 условия:
			\begin{enumerate}
				\item $(\forall A \vdash \alpha|\beta \in P) \Rightarrow ({FIRST(\alpha)\cap{FIRST(\beta)}}=\varnothing)$ 
				\item $(\forall A \vdash \alpha|\beta \in P: \varepsilon \in FIRST(\alpha)) \Rightarrow ({FOLLOW(A)\cap{FIRST(\beta)}}=\varnothing)$
			\end{enumerate}
...
Вернёмся к решению задачи 13.2 в свете леммы 13.5.\\

\textbf{$\blacktriangle$ Задача 13.3} Проверить, что грамматика, задающая язык строк с равным количеством символов $a$ и $b$: $S \vdash aSbS|bSaS|\varepsilon$, не является LL(1).
...
\begin{itemize}
	...
	\item На каждом шаге алгоритма конфигурация алгоритма --- это позиция во входной строке, начиная с которой расположена неразобранная её часть, и стек.
...
{\bf\large $\blacktriangle$ Задачи для самостоятельного решения}
\begin{enumerate}
\item \textbf{a)} Приведите пример $\text{LR}(0)$ грамматики и постройте для неё МП-автомат перенос-свёртка\\
\textbf{b)} Приведите пример $\text{LR}(1)$, но не $\text{LR}(0)$ грамматики.\\
\textbf{с)} Приведите не $\text{LR}(1)$ грамматики.
\item Для грамматик из задачи 1 постройте $LL$-анализаторы и разберите слово длины хотя бы 5, так чтобы в разборе было хотя бы 3 свёртки.
...
\item 	Рассмотрим классический пример \verb|'if, if-else'| в реализации языков программирования:
\begin{align*}
...
\end{enumerate}	
{\bf\large $\blacktriangle$ Задачи для программирование}
\begin{enumerate}
\item Реализуйте $LL(k)$ анализатор.
\end{enumerate}
