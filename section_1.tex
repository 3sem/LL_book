...
\begin{flushleft}
	...
	\begin{tabular}  {p{2.20cm}  p{8.30cm}}
		...
		& \emph{Для LL(k)-грамматики $G = \langle N, \Sigma, P, S \rangle$, для любого её правила вида $A \vdash X_1\dots{X_l} \in P$, если $x \in FIRST_k(FIRST_k(X_1)\cdot{}\dots{\cdot{}{FIRST_k(X_l)}}
			\cdot{FOLLOW_k(A)})$, то ячейка управляющей таблицы $T_k(A, x)$ содержит единственное правило $A \vdash X_1\dots{X_l}$.}.
	\end{tabular}
